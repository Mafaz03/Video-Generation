...
def train_fn(disc, gen, train_loader, opt_disc, opt_gen, l1_loss, bce, g_scaler, d_scaler):
    ...
    for idx, (x,z1,z2,z3,z4,z5,z6,y) in enumerate(loop):
        x = x.to(config.DEVICE)
        z1 = z1.to(config.DEVICE)
        z2 = z2.to(config.DEVICE)
        z3 = z3.to(config.DEVICE)
        z4 = z4.to(config.DEVICE)
        z5 = z5.to(config.DEVICE)
        z6 = z6.to(config.DEVICE)
        y = y.to(config.DEVICE)

        # Train Discriminator
        with torch.amp.autocast("cuda"):
            y_fake = gen(x, z1=z1, z2=z2, z3=z3, z4=z4, z5=z5, z6=z6)
            D_real = disc(x, y)
...
def main():
    ...
    generator = Generator(in_channels=3, inter_images=6, features=64, out_channels=3).to(config.DEVICE)
    ...
    train_dataset = Image_dataset(root_dir=config.TRAIN_DIR, inter_images=6, binarize_output = False, grayscale_all=False)
    train_loader = DataLoader(train_dataset, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=config.NUM_WORKERS)

    g_scaler = torch.amp.GradScaler("cuda")
    d_scaler = torch.amp.GradScaler("cuda")
    
    val_dataset = Image_dataset(root_dir=config.VAL_DIR, inter_images=6, binarize_output = False, grayscale_all=False)
