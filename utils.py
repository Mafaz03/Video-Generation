...
def save_some_examples(gen, val_loader, epoch, folder):
    x,z1,z2,z3,z4,z5,z6,y = next(iter(val_loader))
    x,z1,z2,z3,z4,z5,z6,y = x.to(config.DEVICE), z1.to(config.DEVICE), z2.to(config.DEVICE), z3.to(config.DEVICE), z4.to(config.DEVICE), y.to(config.DEVICE)
    ...
    with torch.no_grad():
        ...
        x, z1, z2, z3, z4, z5, z6 = x*0.5+0.5, z1*0.5+0.5, z2*0.5+0.5, z3*0.5+0.5, z4*0.5+0.5, z5*0.5+0.5, z6*0.5+0.5 # Denormalise

        stacked_images = torch.cat((x,z1,z2,z3,z4,z5,z6,y,y_fake), dim=2)
